...
export async function getUserByEmail(email) {
  const response = await fetch(`${envProvider("VITE_BASE_API_URL")}/authenticate`, {
    method: "GET",
    headers: await getReqHeaders(),
  });
  if (response.status !== 200)
    throw new Error(`Error fetching user with status:${response.status} for email ${email}`);
...
export async function updateUserData({ uuid, wallet_address, role }) {
  ...
  const response = await fetch(`${envProvider("VITE_BASE_API_URL")}/updateuser?${params.toString()}`, {
    method: "GET",
    headers: await getReqHeaders(),
  });
  if (response.status !== 200)
